...
if __name__ == "__main__":
    ...
    code = 'velocity_code'
    import cProfile
    cProfile.run('main(opt)', f"{code}/output.dat")
    import pstats

    with open(f"{code}/output_time.txt", "w") as f:
        p = pstats.Stats(f"{code}/output.dat", stream=f)
        p.sort_stats("time").print_stats()

    with open(f"{code}/output_calls.txt", "w") as f:
        p = pstats.Stats(f"{code}/output.dat", stream=f)
        p.sort_stats(f"calls").print_stats()

# python velocity.py --weights best_3.pt --source "vids/lib.mp4"  --vid-stride 5 --estimate --clf_weights svm/smote_forest.sav --transform transform/smote_stdscaler.bin
